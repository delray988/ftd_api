...
def access_token(method, ftd, payload=None):
   """
   method is either request,refresh or revoke
   ...
   if method == 'request': #if method is 'request' follow the below logic
   ...
   elif method == 'refresh': #if method is 'refresh' follow the below logic
   ...
   elif method == 'revoke': #if method is 'revoke' follow the below logic
   ...
   else: #if none of the methods were 'request', 'refresh', or 'revoke' print to screen the below message
      print("please use 'request', 'refresh' or 'revoke' for the method")

   #Logic to be executed if the user chose an acceptable method and a payload was crafted
   if payload != None:
      #disable warnings
      requests.packages.urllib3.disable_warnings()
      #craft the HTTP POST with the payload based based on the chosen method and diable SSL verification
